			...
			<?php
					...
					if ($cart) {
						...
						foreach($items AS $item) { // Create the $item variable which will act as an individual ID
							...
							if($result == false){
								$mysql_error = mysql_error($con);
								echo "There was a query error: $mysql_error";
							}else{
								//the while loop gets fields for one product row
								while($row=mysqli_fetch_assoc($result)) {
									echo '<tr><td align="center">'.$row['title'].'</td>';
									echo '<td align="center">'.$row['description'].'</td>';
									echo '<td align="center">'.$row['price'].'</td>';
									echo '<td align="center"><a href="my_cart.php?remove_id='.$i.'">Remove From Cart</a></td></tr>';
								}// end while
								$i++; // increments the counter by 1 for next product
							} // end else
						} // foreach loop
					} // end if
			?>
